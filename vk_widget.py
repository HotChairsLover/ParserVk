...
def widget_update():
    ...
    """
    projects = []
    for project in Projects.select().order_by(Projects.players.desc()):  # Создание списка словарей с данными проектов
        proj_data = {
            'проект': project.name,
    ...
    vk_session.method('appWidgets.update', {"code": code, "type": 'table'})  # Запрос к vk api для обновления виджета
